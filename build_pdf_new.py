...
story += [
    ...
    H2('Key Findings'),
    B(
        '<b>Z axis is pure dynamics.</b> Only 0.7% gravitational load (89.59 degrees from vertical). '
    ...
    B(
        ...
        'a 19.55 m/s2 (2.99g) transient hit. Any window based intensity metric '
    ...
    Finding(
        'E03  |  Hour 3.43  |  19.55 m/s2 (3g)  |  Kurtosis 26.0  |  250 ms\n\n'
    ...
    B(
        '<b>Bin std below 0.030 m/s2 means motor off. No CAN bus required.</b> '
        'The overnight park sits at 0.022 m/s2 (sensor noise floor). '
        'Traffic stops with motor running sit at 0.063 to 0.085 m/s2. '
...
story += [
    ...
    P(
        ...
        'cross session artefacts such as sensor mounting angle changes and firmware clock resets '
    ...
    B('<b>Why October 26 specifically?</b> It is the earliest available file, the cleanest '
      ...
      'would require around 50 GB of RAM and hours of I/O time.'),
    SP(4),
    Callout(
...
story += [
    ...
    P(
        ...
        '(acceleration in m/s2, timestamp as Unix epoch float64). '
...
story += [
    ...
    P(
        'Acceleration at +/30 m/s2 needs around 0.001 m/s2 precision. float32 (7 decimal digits) '
        'is more than sufficient and halves the memory cost of the three axis columns. '
        'Timestamps are different: Unix epoch values in 2025 are around 1.76 billion seconds. '
        'float32 maxes at 2 to the power of 24, which is 16.7 million, so it would corrupt every inter sample interval. '
    ...
    P(
        ...
        'The derived rate came back at 800.036 Hz, which is a pass. '
...
story += [
    ...
    P(
        ...
        '(19.62 m/s2) on any single axis. These 37 samples were flagged rather than dropped '
        ...
        '+26.95 and 22.98 m/s2 correspond to actual transient events.',
        'body'),
    Tradeoff(
        'A 3g or 4g ceiling would flag fewer samples but reduce sensitivity to genuine saturation. '
        '2g was chosen as it sits well above normal operation (0 to 0.5g) while remaining below '
...
story += [
    ...
    SP(6),
    P(
        'The X axis mean of 9.399 m/s2 is almost exactly g. X is pointing roughly downward, '
        'carrying most of the gravitational load. '
        'The Z axis mean is 0.070 m/s2, effectively zero. Z is nearly horizontal and carries '
        'no gravity, making it 100% dynamic content by definition. '
        'The Y axis mean of 2.748 m/s2 is persistently negative. This is a fixed mounting offset, '
...
story += [
    ...
    P(
        ...
        'without hidden dependencies on prior state. The cost is around 20 seconds of reload time.',
        'body'),
]
...
story += [
    ...
    Tradeoff(
        ...
        'guarantees no amplitude information is lost. It is the only correct choice for a signal '
...
story += [
    ...
    section_table(
        ...
        [
            ...
            ['min (m/s2)',    '2.658',     '10.726',    '22.975'],
            ['max (m/s2)',    '16.324',    '4.286',      '26.954'],
    ...
    SP(6),
    P(
        'The Z axis std of 2.755 m/s2 is roughly 3x that of X (0.839) and Y (0.933). '
        ...
        'body'),
    P(
        'Kurtosis values of 4.38 to 4.49 across all three axes are tightly clustered at '
    ...
    P(
        ...
        'right skewed (mean 10.194 vs median 9.809 m/s2): impacts add to the magnitude '
        'rather than cancelling across axes, and the median is within 0.002 m/s2 of '
        'standard g, confirming the most common state is the system at rest or cruising smoothly.',
        'body'),
    P(
        'One detail worth flagging: Y axis p99 is 0.273 m/s2, meaning 99% of all Y samples '
...
story += [
    ...
    P(
        ...
        'That is exactly the gravity vector in sensor coordinates, which is orientation.',
        'body'),
    metric_table([
        ('X gravity component', '+9.3986 m/s2  (95.8% of g)'),
        ('Y gravity component', '2.7477 m/s2  (28.0% of g)'),
        ('Z gravity component', '+0.0704 m/s2  (0.7% of g)'),
        ('Recovered magnitude', '9.7923 m/s2  vs standard 9.8066 m/s2'),
        ('Error',               '0.146%  (calibration quality accuracy)'),
        ('X inclination',       '16.30 degrees from vertical'),
        ('Y inclination',       '73.70 degrees from vertical'),
        ('Z inclination',       '89.59 degrees from vertical'),
    ]),
    SP(6),
    P(
        ...
        'body'),
    P(
        'The Z inclination of 89.59 degrees from vertical is the key result. Z carries 0.7% '
    ...
    P(
        ...
        'requiring a 9.399 m/s2 subtraction that is itself sensitive to orientation changes.',
        'body'),
    Tradeoff(
...
story += [
    ...
    metric_table([
        ('Mean magnitude',       '10.194 m/s2  (+0.388 above g)'),
        ('Median magnitude',     '9.809 m/s2   (within 0.002 of standard g)'),
        ('Std',                  '1.061 m/s2'),
        ('Min / Max',            '3.491 / 29.355 m/s2  (peak = 2.99g)'),
        ('Samples above g+1',    '12,422,336  (23.6% of all samples)'),
        ('Samples below g1',    '3,273,503   (6.2%)'),
    ...
    P(
        ...
        'body'),
    P(
        'The peak magnitude of 29.355 m/s2 (2.99g) at hour 3.43 is the single most '
        'important observation in this section. Hour 3 is the calmest, lowest activity '
        'hour in the entire session, with a p95 bin count of only 77 against an expected 180. '
...
story += [
    ...
    P('Gate A:  |bin_mean_magnitude - g| < 0.30 m/s2', 'mono'),
    P('Gate B:  bin_std < 0.10 m/s2', 'mono'),
    P(
    ...
    SP(4),
    section_table(
        ['Segment', 'Duration (s)', 'Mean std (m/s2)', 'Engine state'],
        [
            ['1 (long standby)', '17,710', '0.0222', 'Motor off, at sensor noise floor'],
            ['2 (standby)',     '1,985', '0.0259', 'Motor off'],
            ['3 to 5',          '103 to 278', '0.046 to 0.066', 'Likely motor off, still settling'],
            ['6 to 8 (brief)',    '36 to 60', '0.060 to 0.065', 'Motor likely active, brief stop'],
            ['9 to 15 (micro)',    '4 to 23', '0.063 to 0.085', 'Motor active, brief stops'],
        ],
    ...
    P(
        ...
        'The 4.92 hour overnight standby sits at 0.022 m/s2, the theoretical sensor noise '
        'floor at 800 Hz. The short traffic stops at 0.063 to 0.085 m/s2 carry enough residual '
        'narrow band vibration to indicate the motor was never switched off. '
        'The transition threshold is around 0.030 m/s2: motor state inference '
        'from accelerometer data alone, no CAN bus required.',
        'body'),
    H3('Session structure'),
    B('0 to 1.13h: initial 68 minute drive'),
    B('1.13 to 1.25h: brief stop cluster, four micro segments totalling around 400 seconds'),
...
story += [
    ...
    section_table(
        ['Hour', 'p95 bins', 'max std (m/s2)', 'What this tells us'],
        [
            ['0 to 4',  '66 to 85',  '1.48 to 1.53', 'Quiet baseline, consistent and unremarkable'],
            ...
            ['12 to 13','falling','falling',    'Wind down into parking'],
        ],
        [1.5*cm, 2.5*cm, 3*cm, 9.5*cm]
    ),
    SP(6),
    P(
        'Hours 0 to 4 are calm and consistent, the expected signature of a smooth operational '
        'phase. At hour 5 the p95 count jumps from 85 to 208 (145% increase) and keeps '
        'climbing through hours 6 and 7. The hour 7 peak is driven by max_std (1.550 m/s2): '
        ...
        'Hour 11 peaks differently, with the highest sustained count (381 bins) and a lower max '
        '(1.460 m/s2), indicating continuous high vibration operation rather than hard hits. '
    ...
    P(
        'Active bin std: p50 = 1.231 m/s2, p95 = 1.330 m/s2, p99 = 1.377 m/s2. '
        'The entire range from median to 99th percentile is 0.146 m/s2, which is 11.9% of the '
        'median value. That is unusually tight for 12 hours of varied active operation. '
        'The consequence for threshold based systems: setting a fixed threshold at 1.0 m/s2 '
        'catches everything; at 1.35 m/s2 it catches almost nothing. '
    ...
    P(
        ...
        'around 0.65 m effective wheel diameter): f = v / (pi x d), giving v = 16.7 m/s, approximately 60 km/h. '
        'The 23.83 Hz / 47.27 Hz pair is a near harmonic relationship (47.27 is approximately 2 x 23.83). '
        ...
        'body'),
    Note('Spectrogram covers one 30 minute window (hours 4.59 to 5.09). A full session rolling '
...
story += [
    ...
    P(
        'A global threshold, say flagging any sample where acc_z exceeds 5 m/s2, '
        'treats the same value as equally anomalous at all times. But 5 m/s2 on Z '
    ...
    P(
        ...
        'sensitive to sustained deviations, but not both. The 3g event at hour 3.43 lives '
        'in a single 50 ms window and is invisible at 1 second resolution. '
        'The transition anomalies at hours 1.13 and 1.21 span tens of seconds. No individual sample there is locally remarkable. '
...
story += [
    ...
    P(
        ...
        'body'),
    B('Under Gaussian noise: P(|z| > 5.0) = 5.7 x 10 to the power of 7 per sample'),
    B('Per 1 second window: expected false positives around 0.001'),
    B('Over all 65,825 windows: around 59 expected false positive samples by chance'),
    B('Actual flagged: 2,669 unique samples, of which roughly 2,610 are real'),
    SP(4),
    Tradeoff(
        'K = 3.0 sigma produces around 25,000 false positives by chance, indistinguishable from '
    ...
    P(
        'The NOISE_FLOOR of 0.005 m/s2 as minimum denominator prevents a specific failure: '
        'during the 4.92 hour overnight park the within window std approaches the sensor '
        'noise floor (around 0.022 m/s2). Without a floor, tiny thermal drift would inflate '
    ...
    metric_table([
        ...
        ('Expected by chance',  'around 59 samples  (actual count is 45x higher)'),
    ]),
    SP(6),
    P(
...
story += [
    ...
    metric_table([
        ...
        ('Expected by chance',    'around 4 bins'),
    ]),
    SP(6),
    P(
...
story += [
    ...
    P(
        ...
        'Window: 50 ms non overlapping (40 samples). Energy metric: peak |mag minus g| per window.',
        'body'),
    ...
    B('<b>Peak within the window rather than RMS:</b> RMS averages energy across the window. '
      'A single sample 20 m/s2 spike in a 40 sample window contributes only 1/40th of its '
      'power to the RMS. Peak preserves the maximum instantaneous amplitude.'),
    B('<b>50 ms window:</b> short enough to resolve a typical mechanical impact (20 to 100 ms), '
    ...
    P(
        ...
        '(6.920 m/s2). A fixed absolute threshold was explicitly rejected: '
    ...
    metric_table([
        ...
        ('Peak magnitude range',     '6.920 to 19.548 m/s2'),
    ]),
]
...
story += [
    ...
    section_table(
        ['#', 'Time', 'Dur ms', 'Type', 'Peak m/s2', 'ptpZ', 'Breadth', 'Kurt'],
        [
    ...
    P(
        ...
        'Peak magnitudes span 6.99 to 19.55 m/s2 (2.8x range). Breadth spans 0.584 to 0.978: '
    ...
    Finding(
        '25 seconds into the 18 hour recording. Peak 6.99 m/s2, 7th population percentile. '
        'Z dominant, breadth 0.737, kurtosis 12.7. '
        'Establishes the session baseline character: 50 ms, moderate multi axis spread, '
        'moderate kurtosis. All later events are implicitly compared against it.'),

    SP(4),
    P('<b>E02: The most oblique impact in the set</b>', 'h3'),
    Finding(
        'Breadth 0.978: closest to perfect equipartition across all three axes. '
        'ptpX = 8.85, ptpY = 7.14, ptpZ = 12.18 m/s2. Only 1.7x spread between strongest '
        'and weakest axis, versus more than 3x Z dominance in typical events. '
    ...
    Finding(
        'Peak 19.55 m/s2 at the 100th population percentile. '
    ...
    Finding(
        'E04 (peak 16.52 m/s2, breadth 0.741, kurtosis 13.9) and E05 '
        '(peak 17.13 m/s2, breadth 0.713, kurtosis 10.1) have cosine similarity 0.995 '
    ...
    Finding(
        ...
        'Highest total impulse in the full 6,019 event population at 1.1995 m/s2 x s. '
        'Peak 8.68 m/s2 is moderate. The energy accumulates across 21 detection bins. '
    ...
    Finding(
        ...
        'with no observable build up phase. Combined with breadth 0.860 and multi axis response '
        '(ptpX = 7.78, ptpY = 8.78, ptpZ = 25.24), this is consistent with a direct lateral or '
    ...
    Finding(
        'Second highest peak at 19.30 m/s2, delivered in a single 50 ms window. '
        'Instantaneous energy density: 386 m/s2 per second. Compare to E03 at 19.55 m/s2 over 250 ms '
        '= 78 m/s2 per second. E08 delivers nearly equal peak intensity in one fifth the time. '
    ...
    Finding(
        'Lowest breadth at 0.584. Z axis ptpZ = 31.12 m/s2. Next highest ptpX = 4.09 m/s2, '
    ...
    Finding(
        'Latest event at hour 13.36, which is 13.35 hours after E01. '
        'Peak 7.07 m/s2 versus E01 at 6.99 m/s2: a 1.1% difference. '
...
story += [
    ...
    P(
        'The six features have very different scales: peak_mag ranges 6.99 to 19.55, '
        'dur_ms ranges 50 to 1,050, breadth ranges 0.584 to 0.978. '
    ...
    metric_table([
        ...
        ('Near twin pair',       'E04 vs E05  cosine approximately 0.995  (1.92 hours apart)'),
        ('Most dissimilar pair', 'E07 vs E09  cosine = 0.7502'),
    ]),
    SP(6),
    P(
        'The E04 to E05 near twin at cosine 0.995 reinforces the per event finding: two shocks '
    ...
    P(
        ...
        'as anomalous just because their peak_mag ranks highest would be circular. '
...
story += [
    ...
    B('<b>No ground truth.</b> All event characterisation is intrinsic. No GPS, operator '
      'annotations, or external data to confirm the nature of E03, E06, or the E04 to E05 '
      'recurring signature.'),
    ...
    B('<b>Bin std below 0.030 m/s2 reliably indicates motor off.</b> '
    ...
    B('<b>Track kurtosis as a deployment health metric.</b> Session baseline is 4.38 to 4.49 '
      'across all axes. If Z axis kurtosis climbs above 6 to 8 on a known route without a '
    ...
    B('<b>Dual scale anomaly pipeline running in parallel.</b> '
      'Fine scale (1 second Z score at 5 sigma) catches instantaneous spikes like E03. '
      'Coarse scale (60 second rolling MAD at 4 sigma) catches sustained deviations and '
      'phase transitions like hours 1.13 to 1.21. '
    ...
    B('<b>Motor state inference from bin std.</b> '
      ...
      'Transition below 0.030 m/s2 means motor off event; above 0.060 m/s2 means motor on. '
    ...
    B('<b>Cross session normalisation layer.</b> '
      ...
      'normalise peak magnitude by (session_p99 minus session_p50) to produce a dimensionless '
    ...
    B('<b>Kurtosis health monitor (background process).</b> '
      ...
      'Baseline for this device: 4.38 to 4.49. '
