...
### IOV Accelerometer Signal — Part 2 Signal Characterization
...
## 3. The Kurtosis Values Are Consistent Across All Axes — and That Consistency Is the Observation

All three axes return kurtosis in the range **4.38–4.49**, tightly clustered around 4.43. This uniformity across geometrically independent axes rules out axis-specific artifacts. The elevated kurtosis (~48% above the Gaussian baseline of 3.0) is a property of the system's dynamics, not the sensor.

Kurtosis in this range is the fingerprint of **moderate impulsive content**: recurring short-duration, high-amplitude events layered on top of a broadly Gaussian vibration background. Not catastrophic (kurtosis > 10 would indicate structural distress), not clean (kurtosis = 3.0 would indicate pure background vibration noise). Think surface impacts, step transitions, mechanical transitions — events that last milliseconds but happen regularly enough over 18 hours to systematically fatten all three tail distributions.

The product implication is direct: kurtosis is cheap to compute (one pass over a 1-second buffer), orientation-independent in the sense that all three axes are equally elevated, and sits at a value (~4.4) that's actionable as a deployment baseline. A system where kurtosis climbs above 6–8 on the Z axis over time is telling you something changed — surface conditions, mounting wear, or component condition.
...
- **0–1.13h**: initial continuous active phase (~68 minutes)
- **1.13–1.25h**: brief stop cluster — 4 micro-segments totaling ~400 seconds (scheduled stops, loading, or servicing)
- **1.25–8.44h**: the dominant feature — **7.19 hours of continuous operation without a single stop**
- **8.44h**: a 76-second pause (two back-to-back segments of 60s and 16s)
- **8.46–11.79h**: continued active operation (~3.33 hours)
- **11.79–12.58h**: fragmented wind-down — 6 short segments (4–120 seconds each) interspersed with brief active phases
- **12.70–13.25h**: 33-minute stationary period (motor off, mean bin-std = 0.026 m/s²)
- **13.25–13.37h**: brief 7-minute return phase (only 425 active seconds in the entire hour)
- **13.37–18.28h**: terminal 4.92-hour standby (motor off, mean bin-std = 0.022 m/s²)
...
|---------|----------|-------------|----------------------|
| Long standby (#1, #2) | 4.92h, 33min | 0.022–0.026 m/s² | Motor off — at/near sensor noise floor |
| Medium stops (#3–#5) | 1.7–4.6 min | 0.046–0.066 m/s² | Likely motor off, not fully settled |
| Short stops (#6–#15) | 4–60 seconds | 0.063–0.085 m/s² | Motor likely active, brief stop |

The theoretical sensor noise floor at 800 Hz is ~0.02–0.05 m/s². The long standby periods at 0.022 m/s² are sitting at the absolute floor — these are dead-silent observations. The 60-second stops at 0.065 m/s² carry enough residual vibration energy to suggest the motor had not been switched off.
...
- Hours 0–4: **66–85** (consistent low baseline)
- Hours 5–7: **208 → 248 → 335** (progressive ramp, +304% from baseline by hour 7)
- Hour 8: **169** (sudden drop, 76-second stop at 8.44h)
- Hours 9–10: **181 → 228** (renewed climb)
...
## 13. No Fixed Structural Resonances Is Itself the Baseline — And It's Fragile
...
But it's also worth understanding what "no structural resonances" means in this analysis: the spectrogram covers one **30-minute clip** out of 7.19 hours of continuous active operation. Structural resonances that only appear at specific operational speed ranges, or under specific load conditions, would be missed. The analysis is suggestive, not exhaustive.
